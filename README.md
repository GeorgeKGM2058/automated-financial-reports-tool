...
A Python pipeline that pulls live financial statements from Yahoo Finance, computes key ratios, runs a two-stage DCF valuation with live country risk premiums, and exports a PDF report, PowerPoint deck, and CSV per ticker.
...
Runs in Annual mode by default. TTM mode (trailing twelve months, summing the 4 most recent quarterly filings) is available by changing `use_ttm=False` to `use_ttm=True` in the last cell. Quarterly data coverage on yfinance is thinner for non-US tickers, so TTM mode works most reliably on US stocks.
...
Any ticker on Yahoo Finance. By default already includes the following:
| Market | Examples |
|---|---|
